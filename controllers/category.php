...
class K2toflexiControllerCategory extends JControllerLegacy {
	...
	public function category($data = false)
	{
		if ($data == false ){
		...
		$data = json_decode($data);
  		$task = $data->{'task'};
  		$sql = $data->{'sql'};
		$valuesjsons = $this->getModel('category')->category($task, $sql);
	...
	public function firstmigrateCategory()
	{
		$response = json_encode(array("success" => true, "message" => "Loading...", "task" => 'insertCategory', "sql" => ''));
