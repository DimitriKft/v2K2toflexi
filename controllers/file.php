...
class K2toflexiControllerFile extends JControllerLegacy {
	...
	public function file($data = false)
	{
		...
		$data = json_decode($data);
  		$task = $data->{'task'};
  		$sql = $data->{'sql'};
		$valuesjsons = $this->getModel('file')->file($task, $sql);
	...
	public function firstmigrateFile()
	{
		$response = json_encode(array("success" => true, "message" => "Loading...", "task" => 'insertFiles', "sql" => ''));
