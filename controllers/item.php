...
class K2toflexiControllerItem extends JControllerLegacy {
	...
	public function item($data = false)
	{
		if ($data == false ){
		...
		$data = json_decode($data);
  		$task = $data->{'task'};
  		$sql = $data->{'sql'};
		$valuesjsons = $this->getModel('item')->item($task, $sql);
	...
	public function firstmigrateItem()
	{
		$response = json_encode(array("success" => true, "message" => "Loading...", "task" => 'insertItem', "sql" => ''));
