...
$document->addStyleSheet("./components/com_k2toflexi/assets/css/style.css",'text/css',"screen");
		JText::script('COM_K2TOFLEXI_ANALYSISFINISHED');
...
<div id="contenu">
	<div id="titre" class="page-header"><h1><?php echo 'Analyse des données K2'; ?></h1></div>
	<div id="k2toflexi"><button id="renderbutton" type="button" class="btn btn-primary" value="renderbutton" onclick="waitForFirstData();"><?php echo 'Lancer l\'analyse'; ?></button></div>
		...
		success: function(data){
			...
			if (data.task == false){
				...
				jQuery("#loading").append(
					Joomla.JText._('COM_K2TOFLEXI_ANALYSISFINISHED')
				);
	        }else{
				NextData(data.message);
 				scrollmove();
